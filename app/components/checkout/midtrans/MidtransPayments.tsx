...
const LOGOS = {
  ...
  shopeepay:
    "https://upload.wikimedia.org/wikipedia/commons/f/fe/ShopeePay_logo.svg",
  qris: "https://upload.wikimedia.org/wikipedia/commons/a/a2/Logo_QRIS.svg",
  indomaret:
    "https://upload.wikimedia.org/wikipedia/commons/4/4d/Indomaret_logo.svg",
  alfamart:
  ...
  atm_bersama:
    "https://upload.wikimedia.org/wikipedia/commons/4/4c/ATM_Bersama_logo.svg",
            ...
            <div>
              ...
              <p className="text-xs text-red-700 mt-1">
                {typeof paymentError === "string"
                  ? paymentError
                  : paymentError.message || "An error occurred"}
              </p>
            </div>
          </div>
        )}
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2 overflow-hidden px-1">
                    {cat.logos.map((logo, i) => (
                      <div
                        key={i}
                        className="h-10 w-auto flex items-center bg-white px-2 border border-gray-50 rounded-md shadow-sm"
                      >
                        <img
                          src={logo}
                          alt="logo"
                ...
                <div className="p-4 bg-gray-50/50 border border-gray-100 rounded-xl ml-4">
                  {cat.id === "bank_transfer" && (
                    <div className="grid grid-cols-2 gap-2">
                      {(Object.keys(BANK_LABELS) as MidtransBankType[]).map(
                        (bank) => (
                          <button
                            key={bank}
                            type="button"
