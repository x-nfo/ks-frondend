...
function getProductColorHexes(item: any, facetLookup: Map<string, any>) {
    const productFacets = item.facetValueIds?.map((id: string) => facetLookup.get(id)).filter(Boolean) || [];
    const colorFacets = productFacets.filter((f: any) =>
...
export default function CollectionSlug({ loaderData }: Route.ComponentProps) {
    ...
    const facetLookup = useMemo(() => {
        const map = new Map<string, { name: string; code: string; facetName: string; facetCode: string }>();
    ...
    const availableColors = useMemo(() => {
        const colorFacets = facets
            ?.filter((f: any) =>
                ['color', 'colour', 'warna'].includes(f.facetValue?.facet?.code?.toLowerCase()) ||
                ['color', 'colour', 'warna'].includes(f.facetValue?.facet?.name?.toLowerCase())
            ) || [];
        ...
        return Array.from(colorsMap.entries()).map(([hex, name]) => ({ name, hex }));
    }, [facets]);

    const { minPriceLimit, maxPriceLimit } = useMemo(() => {
        if (!initialProducts.length) return { minPriceLimit: 0, maxPriceLimit: 100000000 };

        const prices = initialProducts.map((p: any) => p.priceWithTax.min || p.priceWithTax.value);
        return {
    ...
    const filteredProducts = useMemo(() => {
        let result = initialProducts.filter((p: any) => {
            ...
            const productFacets = p.facetValueIds?.map((id: string) => facetLookup.get(id)).filter(Boolean) || [];

            // Colors (Client-side)
            if (filters.colors.length > 0) {
                const productHexes = getProductColorHexes(p, facetLookup);
                if (!productHexes.some(hex => filters.colors.includes(hex))) return false;
            }

            // Category (Client-side)
            if (filters.category.length > 0) {
                const productCategories = productFacets
                    .filter((f: any) => f.facetCode === 'category' || f.facetName.toLowerCase() === 'category')
                    .map((f: any) => f.name);

                // We check if the product has ANY of the selected categories
                if (!productCategories.some((c: string) => filters.category.includes(c))) return false;
            }
                ...
                <div className="flex flex-col lg:flex-row gap-8 relative items-start">
                    ...
                    <div className="hidden lg:block w-64 shrink-0 h-fit sticky top-40">
                        <FilterSidebar
                            ...
                            onClearFilters={() => setFilters({ category: [], colors: [], minPrice: minPriceLimit, maxPrice: maxPriceLimit })}
                    ...
                    <div className="flex-1 min-h-[50vh]">
                        {filteredProducts.length > 0 ? (
                            <div className={`grid gap-x-4 gap-y-12 md:gap-x-12 md:gap-y-32 ${gridCols === 3 ? 'grid-cols-2 lg:grid-cols-3' : 'grid-cols-2'}`}>
                                {filteredProducts.map((item: any, idx: number) => {
                                    const productFacets = item.facetValueIds?.map((id: string) => facetLookup.get(id)).filter(Boolean) || [];
                                    const categoryFacet = productFacets.find((f: any) =>
                            ...
                            <div className="py-40 text-center border-t border-karima-brand/5">
                                <p className="text-serif italic text-2xl text-karima-brand/40">No products match your filters.</p>
                                <button
                                    onClick={() => setFilters({ category: [], colors: [], minPrice: minPriceLimit, maxPrice: maxPriceLimit })}
    ...
    return (
        <div className="bg-white min-h-screen pt-48 flex items-center justify-center text-center px-6">
            <div className="space-y-8">
                <h1 className="text-7xl md:text-9xl font-serif text-karima-brand italic opacity-10">404</h1>
                <h2 className="text-2xl font-serif italic text-karima-brand">The collection you seek is elsewhere.</h2>
                <div className="w-12 h-[1px] bg-karima-brand/20 mx-auto"></div>
                <Link to="/" className="block text-[10px] uppercase tracking-[0.4em] font-bold text-karima-brand">Back to Karima</Link>
            </div>
        </div>
    );
}
