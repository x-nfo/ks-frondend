...
export function ContactStep() {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const { emailAddress, firstName, lastName } = Object.fromEntries(
      formData.entries(),
    );
    ...
    if (isSubmitting && localFetcher.state === "idle") {
      ...
      if (fetcherError) {
        ...
        if (errorCode === "EMAIL_ADDRESS_CONFLICT_ERROR") {
          setErrorMessage(
            'This email is already registered. If you have shopped with us as a guest before, you might not have a password yet. Please use the "Forgot Password" link to set one.',
          );
          ...
          setErrorMessage(
            fetcherError.message || "An error occurred. Please try again.",
          );
        }
        return;
      }
      ...
      const propSuccess = !!(
        activeOrder?.customer?.emailAddress && activeOrder?.customer?.firstName
      );
  ...
  const renderHeader = () => (
    ...
    >
      <div className="flex items-center gap-x-4">
        <h2 className="text-base font-bold text-karima-brand font-sans leading-tight">
          1. Email
        </h2>
        {!activeCustomer && (
