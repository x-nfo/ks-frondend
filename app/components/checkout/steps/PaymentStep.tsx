...
export function PaymentStep() {
  ...
  return (
    <div
      className={`bg-transparent sm:rounded-none transition-all duration-300 overflow-hidden mb-2 ${isActive ? "" : ""} ${isDisabled ? "opacity-40 grayscale" : ""}`}
    >
      <div className="py-6 px-0 flex items-center justify-between border-b border-gray-100">
        ...
        {stepsStatus.payment === "completed" && (
          <div className="text-karima-brand font-bold text-xs uppercase tracking-widest font-sans">
            COMPLETED
          </div>
        )}
      </div>
      ...
      {isActive && (
        <div className="pb-6 pt-4 px-0 lg:px-0 space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
          ...
          <div
            className={clsx(
              "space-y-6",
              isBillingSaved && "opacity-50 pointer-events-none",
            )}
          >
            <BillingAddressSection
              availableCountries={
                availableCountries.map((c) => ({ ...c, id: c.code })) as any
              }
                ...
                {useSameAsShipping ? (
                  <>
                    ...
                    <input
                      type="hidden"
                      name="countryCode"
                      value={shippingAddress?.countryCode || "ID"}
                    />
                    <input
                      type="hidden"
                      name="phoneNumber"
                  ...
                  <>
                    <input
                      type="hidden"
                      name="fullName"
                      value={billingAddressData?.fullName || ""}
                    />
                    <input
                      type="hidden"
                      name="streetLine1"
                      value={billingAddressData?.streetLine1 || ""}
                    />
                    <input
                      type="hidden"
                      name="streetLine2"
                      value={billingAddressData?.streetLine2 || ""}
                    />
                    <input
                      type="hidden"
                      name="city"
                    ...
                    <input
                      type="hidden"
                      name="countryCode"
                      value={billingAddressData?.countryCode || "ID"}
                    />
                    <input
                      type="hidden"
                      name="phoneNumber"
              ...
              <div className="mt-8">
                <fetcher.Form method="post" action="/api/active-order">
                  ...
                  <button
                    type="submit"
                    disabled={
                      !canCompleteOrder || isSubmitting || isGlobalLoading
                    }
                ...
                {fetcher.data?.error && (
                  <p className="mt-4 text-sm text-red-600 font-bold italic text-center">
                    ...
                    {typeof fetcher.data.error === "string"
                      ? fetcher.data.error
                      : fetcher.data.error.message || "Payment failed"}
                  </p>
                )}
              </div>
            </div>
