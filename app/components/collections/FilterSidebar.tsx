...
export function FilterSidebar({
    ...
    return (
        <div>
            ...
            {/* Sidebar Container */}
            <div className={`bg-white border-r border-karima-brand/5 shadow-2xl transition-transform duration-500 fixed inset-y-0 left-0 z-[2001] w-[85vw] md:w-[320px] lg:relative lg:inset-auto lg:z-auto lg:w-full lg:h-auto lg:shadow-none lg:border-none lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
                                ...
                                <div className="space-y-4">
                                    {availableCategories.map((cat: string) => (
                                        <label key={cat} className="flex items-center gap-3 cursor-pointer group">
                                            ...
                                            <span className={`text-micro uppercase tracking-widest transition-colors ${filters.category.includes(cat) ? 'text-karima-brand font-medium' : 'text-karima-ink/60 group-hover:text-karima-brand'}`}>
                                                {cat}
                                            </span>
                                            <input
                                                type="checkbox"
                        ...
                        <div>
                            ...
                            <div className="space-y-4">
                                <input
                                    ...
                                    min={minPriceLimit}
                                    max={maxPriceLimit}
                                    step={Math.max(1, Math.round((maxPriceLimit - minPriceLimit) / 100))}
                                    value={Math.min(maxPriceLimit, Math.max(minPriceLimit, filters.maxPrice || maxPriceLimit))}
                                ...
                                <div className="flex justify-between text-micro font-serif text-karima-brand italic">
                                    ...
                                    <span>Rp {Math.min(maxPriceLimit, filters.maxPrice || maxPriceLimit).toLocaleString('id-ID')}</span>
                                </div>
                            </div>
                        </div>
