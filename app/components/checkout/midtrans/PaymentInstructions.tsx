...
export function PaymentInstructions({
  ...
  const billKey =
    getProp(paymentData, "billKey") || getProp(paymentData, "bill_key");
  const billerCode =
    getProp(paymentData, "billerCode") || getProp(paymentData, "biller_code");
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 text-left">
        ...
        <div className="space-y-4">
          <div className="bg-gray-50 rounded-lg p-5 border border-gray-100">
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 font-sans">
              Biller Code
            </p>
  ...
  if (paymentType === "bank_transfer" || paymentType === "permata") {
    const vaNumbers =
      getProp(paymentData, "vaNumbers") || getProp(paymentData, "va_numbers");
    const permataVaNumber =
      getProp(paymentData, "permataVaNumber") ||
      getProp(paymentData, "permata_va_number");
    const vaNumberDirect =
      getProp(paymentData, "vaNumber") || getProp(paymentData, "va_number");

    // Permata specific logic: often returned in permataVaNumber even for bank_transfer type
    const bank = (
      getProp(vaNumbers?.[0], "bank") ||
      getProp(paymentData, "bank") ||
      ""
    ).toLowerCase();

    if (bank === "permata" || paymentType === "permata" || permataVaNumber) {
      const finalPermataVa =
        permataVaNumber ||
        vaNumberDirect ||
    ...
    if ((vaNumbers && vaNumbers.length > 0) || vaNumberDirect) {
      ...
      const bankName =
        BANK_LABELS[bank as keyof typeof BANK_LABELS] || bank || "Bank";
      const vaNumber =
        vaNumberDirect || getProp(va, "vaNumber") || getProp(va, "va_number");
