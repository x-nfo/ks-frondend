...
export function DeliveryStep() {
  ...
  const availableShippingOptions = eligibleShippingMethods.map((method) => {
    ...
    return {
      ...
      service:
        m.metadata?.service ||
        (m.name.includes(" ") ? m.name.split(" ").pop() : "STD"),
  ...
  const savedDestinationId = (activeOrder?.shippingAddress as any)?.customFields
    ?.rajaOngkirDestinationId;
  const [selectedDestinationId, setSelectedDestinationId] = useState<
    string | number | null
  >(savedDestinationId || null);
    ...
    >
      ...
      {isActive && (
        <div className="pb-10 pt-4 px-0 lg:px-0 animate-in fade-in slide-in-from-top-4 duration-500 space-y-8">
          ...
          {(selectedDestination ||
            (selectedDestinationId && availableShippingOptions.length > 0)) && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
              ...
              <RajaOngkirShippingSelector
                ...
                selectedOptionId={
                  selectedShippingOptionId || currentShippingMethod?.id || null
                }
