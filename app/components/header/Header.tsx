...
export function Header({
  ...
  const isSignedIn = !!(
    data &&
    data.activeCustomer &&
    data.activeCustomer.activeCustomer?.id
  );
  ...
  const textColorClass = getResponsiveColor(
    "text-karima-ink",
    "text-karima-base",
  );
  const logoColorClass = getResponsiveColor(
    "text-karima-brand",
    "text-karima-base",
  );
  const bgLinkHover = getResponsiveColor("bg-karima-brand", "bg-karima-base");
  const accentColorClass = getResponsiveColor(
    "text-karima-accent",
    "text-karima-base/70",
  );
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-6 md:px-12 max-w-7xl flex items-center justify-between">
          ...
          <div className={`flex items-center gap-6 md:gap-8 ${textColorClass}`}>
            ...
            >
              ...
              {wishlistCount > 0 && (
                <span
                  className={`absolute -top-2 -right-2 ${cartBadgeBg} ${cartBadgeText} text-[10px] w-5 h-5 flex items-center justify-center rounded-full font-serif italic`}
                >
                  {wishlistCount}
                </span>
              )}
            ...
            >
              ...
              {cartQuantity > 0 && (
                <span
                  className={`absolute -top-2 -right-2 ${cartBadgeBg} ${cartBadgeText} text-[10px] w-5 h-5 flex items-center justify-center rounded-full`}
                >
                  {cartQuantity}
                </span>
              )}
            ...
            >
              {isMobileMenuOpen ? (
                <X size={24} strokeWidth={2} className="text-karima-brand" />
              ) : (
      ...
      <div
        className={`fixed inset-0 h-[100dvh] bg-karima-base z-[900] flex flex-col ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
            ...
            <div className="flex flex-col gap-4">
              ...
              >
                ...
                <ChevronDown
                  size={24}
                  strokeWidth={1}
                  className={`transition-transform duration-500 ${isMobileCategoriesOpen ? "rotate-180 text-karima-gold" : "text-karima-brand"}`}
                />
              </button>

              {/* Nested Categories */}
              <div
                className={`flex flex-col gap-4 pl-6 overflow-hidden transition-all duration-500 ${isMobileCategoriesOpen ? "max-h-[500px] opacity-100" : "max-h-0 opacity-0"}`}
              >
                {facetCategories.map((cat: string) => (
                  <Link
                    key={cat}
          ...
          {isMobileMenuOpen && (
            <div className="flex flex-col gap-4 text-left border-t border-karima-brand/10 pt-10 mr-12">
              <Link
                to="/account"
                onClick={() => setIsMobileMenuOpen(false)}
                className="text-karima-ink/60 hover:text-karima-brand uppercase tracking-[0.15em] text-xs font-medium text-left"
              >
                My Account
              </Link>
              <Link
          ...
          {isMobileMenuOpen && (
            <div className="flex items-center gap-8 mt-4">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-karima-brand hover:text-karima-gold transition-colors"
              >
