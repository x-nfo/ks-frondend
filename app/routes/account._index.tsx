...
import { useActionData, useLoaderData, useNavigation, data, redirect, Form } from "react-router";
...
export async function action({ request }: Route.ActionArgs) {
    ...
    if (intent === FormIntent.UpdateEmail) {
        ...
        const { password: requestPassword, email: newEmailAddress } = submission.value;
        ...
        if (updateResult?.__typename !== 'Success') {
            const errorMsg = (updateResult && 'message' in updateResult)
    ...
    if (intent === FormIntent.UpdateDetails) {
        ...
        try {
            ...
            if (!result || result.id === undefined) {
                return data({
                    intent,
                    payload: submission.reply({ formErrors: ['Failed to update account details.'] })
                }, { status: 400 });
            }
            ...
            return data({
                intent,
                payload: submission.reply({ formErrors: [error?.message || 'An error occurred while updating details.'] })
            }, { status: 500 });
        }
    }
...
export default function AccountDetails() {
    ...
    const [emailForm, emailFields] = useForm({
        id: 'email-form',
        lastResult: actionData?.intent === FormIntent.UpdateEmail ? actionData.payload : null,
        onValidate({ formData }) {
    ...
    const [detailsForm, detailsFields] = useForm({
        id: 'details-form',
        lastResult: actionData?.intent === FormIntent.UpdateDetails ? actionData.payload : null,
        defaultValue: {
            title: title ?? '',
            firstName,
            lastName,
    ...
    // Derived
    const emailSavedResponse = actionData?.intent === FormIntent.UpdateEmail && actionData.success ? { newEmailAddress: actionData.newEmailAddress } : null;
                        ...
                        <div className="gap-4 grid sm:grid-cols-2">
                            ...
                            {isEditing ? (
                                <>
                                    <div>
                                        <Input
                                            {...getInputProps(detailsFields.firstName, { type: 'text' })}
                                            label="First Name"
                                            required
                                    ...
                                    <div>
                                        <Input
                                            {...getInputProps(detailsFields.lastName, { type: 'text' })}
                                            label="Last Name"
                                            required
                                ...
                                <div>
                                    <h3 className="text-sm text-karima-ink/60">
                                        Full Name
                                    </h3>
                            ...
                            <div>
                                {isEditing ? (
                                    <Input
                                        {...getInputProps(detailsFields.phoneNumber, { type: 'tel' })}
                                        label="Phone Nr."
