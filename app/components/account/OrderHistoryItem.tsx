    ...
    >
      ...
      <div
        className="p-4 lg:p-6
            ...
            bg-karima-base/30 border-b border-karima-base
        "
      >
        {/* Infos */}
        <div className="flex flex-col md:flex-row gap-4 md:gap-8 lg:gap-16 text-sm">
          {/* Info - Date */}
          <div>
            <span className="block font-medium text-karima-ink">Date</span>
            <span
              className="text-karima-ink/60"
          ...
          {/* Info - Total sum */}
          <div>
            <span className="block font-medium text-karima-ink">Total Sum</span>
            <span className="text-karima-ink/60">
              <Price
        ...
        <div className="gap-4 lg:gap-6 flex flex-col items-end self-stretch justify-between md:flex-row md:items-center self-start">
          ...
          <div className="flex" role="group">
            <Button
              title="Actions"
              className="bg-white text-sm rounded-r-none border-r-0 border-karima-base text-karima-ink"
            >
              <span className="text-xs hidden">Actions</span>
              <EllipsisVerticalIcon className="w-5 h-5" />
            </Button>
            <Button
              className="bg-white text-sm rounded-l-none border-karima-base text-karima-ink"
      ...
      {isExpanded && (
        <div className="flex flex-col">
          {order?.lines.map((line) => (
            <div
              key={line.id}
              className="p-4 lg:p-6 border-b border-karima-base flex flex-row gap-8 justify-between group"
            >
              {/* Product */}
              <div className="inline-flex justify-center items-center justify gap-4">
                ...
                <span className="flex flex-1 flex-col gap-0">
                  ...
                  <Link
                    to={`/products/${line.productVariant.product.slug}`}
                    className="text-karima-ink text-sm font-semibold line-clamp-3 md:line-clamp-2 max-w-md hover:text-karima-ink/50"
                  ...
                  {/* Price and quantity */}
                  <button
                    className="inline-flex gap-2 items-center w-fit text-karima-ink/60 text-sm mt-1"
          ...
          <div className="p-2 lg:py-3 lg:px-6 gap-2 lg:gap-6 grid grid-cols-2 sm:flex justify-end items-center">
            {order?.fulfillments?.map((f, i) => (
              <Button
                key={f.id}
                onClickCapture={() =>
                  alert(`Tracking code: "${f.trackingCode}"`)
                }
                className="text-xs"
              >
