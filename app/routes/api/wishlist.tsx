import { type ActionFunctionArgs, type LoaderFunctionArgs, data, redirect } from "react-router"; // React Router 7
import {
    addToWishlist,
    removeFromWishlist,
...
export async function action({ request }: ActionFunctionArgs) {
    ...
    try {
        if (action === "add" && productVariantId) {
            ...
            if (isFetchRequest) {
                return data({ success: true, item: result.addToWishlist }, { headers: (result as any)._headers });
            }
            ...
            if (itemId) {
                ...
                if ((removeResult as any)._headers) {
                    (removeResult as any)._headers.forEach((v: string, k: string) => finalHeaders.set(k, v));
                }
                ...
                if (wishlist._headers) {
                    wishlist._headers.forEach((v: string, k: string) => finalHeaders.set(k, v));
                }
                if (item) {
                    const removeResult = await removeFromWishlist((item as any).id, { request });
                    if ((removeResult as any)._headers) {
                        (removeResult as any)._headers.forEach((v: string, k: string) => finalHeaders.set(k, v));
                    }
                }
        ...
        try {
            ...
            if (parsedError.extensions?.code === "FORBIDDEN" || errorMessage.toLowerCase().includes("forbidden") || errorMessage.toLowerCase().includes("authorized")) {
                isAuthError = true;
            }
        } catch (e) {
            // Fallback to string matching if not JSON
            errorMessage = error?.message || errorMessage;
            isAuthError = errorMessage.toLowerCase().includes("forbidden") || errorMessage.toLowerCase().includes("authorized");
        }
...
export async function loader({ request }: LoaderFunctionArgs) {
    try {
        ...
        return data({
            ...
            variantIds: wishlist.activeCustomerWishlist.map((i: any) => i.productVariant.id),
        });
    } catch (error) {
