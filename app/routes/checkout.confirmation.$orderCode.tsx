...
export async function loader({ params, request, context }: Route.LoaderArgs) {
  ...
  const hasAuthToken =
    cookieHeader.includes("authToken") ||
    cookieHeader.includes("vendure_session");
  console.log(`[confirmation loader] orderCode: ${orderCode}`);
  console.log(
    `[confirmation loader] Cookie header present: ${cookieHeader.length > 0}, hasAuthToken: ${hasAuthToken}`,
  );

  try {
    const order = await getOrderByCode(orderCode, options);
    console.log(
      `[confirmation loader] getOrderByCode result: ${order ? `FOUND (state: ${order.state})` : "NULL"}`,
    );
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.log(
          `[confirmation] midtransPaymentData raw response:`,
          JSON.stringify(json),
        );
        ...
        if (paymentStatus) {
          ...
          console.log(
            `[confirmation] paymentStateFromQuery:`,
          ...
          console.log(
            `[confirmation] midtransPaymentData returned null/undefined. errors:`,
            JSON.stringify(json.errors),
          );
        }
      } else {
        console.error(
          `[confirmation] midtransPaymentData HTTP error:`,
          res.status,
          res.statusText,
        );
      }
      ...
      console.error(
        "Failed to fetch Midtrans payment data via custom query:",
        e,
      );
    }

    console.log(
      `[confirmation] Successfully fetched order:`,
      order?.code,
      order?.state,
    );
    return {
...
export function ErrorBoundary({ error }: any) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-16 text-center">
      ...
      <h1 className="text-2xl font-sans font-bold text-karima-brand mb-2">
        Something went wrong
      </h1>
      <p className="text-karima-ink/70">
        We encountered an error while displaying your order confirmation. Your
        order may have been processed successfully.
      </p>
      ...
      <div className="mt-8 space-x-4">
        <a
          ...
          className="text-karima-accent font-bold underline hover:text-karima-brand"
        >
          Check Order History
        </a>
        <a
...
export default function CheckoutConfirmation() {
  const { order, orderCode, error, paymentMetadata, paymentStateFromQuery } =
  ...
  const isSettledFromOrder =
    midtransPayment?.state === "Settled" ||
    midtransPayment?.state === "PartiallySettled";
  const isSettledFromQuery =
    paymentStateFromQuery === "Settled" ||
    paymentStateFromQuery === "PartiallySettled";
  const isSettled = isSettledFromOrder || isSettledFromQuery;
  const isDeclined =
    midtransPayment?.state === "Declined" ||
    midtransPayment?.state === "Cancelled" ||
    paymentStateFromQuery === "Declined" ||
    paymentStateFromQuery === "Cancelled";
  ...
  useEffect(() => {
    const hasMidtrans = order?.payments?.some((p) =>
      p.method.includes("midtrans"),
    );
  ...
  if (!order && !error && orderRetries < maxOrderRetries) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-16 text-center">
        <ArrowPathIcon className="w-10 h-10 text-karima-brand mx-auto mb-4 animate-spin" />
        <p className="text-karima-ink/70 font-medium">
  ...
  if (error || !order) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-16 text-center">
        ...
        <p className="text-karima-ink/70">
          Order not found or still processing. Please check your order history.
        </p>
        <a
          href="/account/history"
          className="mt-8 inline-block text-karima-accent font-bold underline hover:text-karima-brand"
        >
          Check Order History
        </a>
      </div>
        ...
        <div className="bg-white/80 shadow-xl rounded-3xl overflow-hidden border border-karima-brand/10 backdrop-blur-sm">
          ...
          <div className="p-6 sm:p-12 text-center border-b border-karima-brand/10">
            ...
            <p className="text-sm font-bold text-karima-gold uppercase tracking-widest">
              Order Number:{" "}
              <span className="text-karima-ink">{order.code}</span>
            </p>
          </div>
          ...
          <div className="p-6 sm:p-12 space-y-10">
              ...
              <div className="bg-red-50 border border-red-100 rounded-2xl p-6 flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-red-700 mb-4">
                    Your transaction failed. You can try to checkout again with
                    another payment method.
                  </p>
                  <a
                    href="/checkout"
                    className="inline-flex items-center gap-2 bg-black hover:bg-karima-brand text-white px-8 py-4 rounded-none text-sm font-black uppercase tracking-widest transition-all"
                  >
            ...
            <section className="pt-6 border-t border-karima-brand/10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 text-sm">
                <div>
                  <p className="font-black text-karima-brand uppercase tracking-tighter mb-2">
                    Shipping Address
                  </p>
                  <div className="text-karima-ink/80 space-y-1">
                    <p className="font-bold text-karima-ink">
                ...
                <div>
                  <p className="font-black text-gray-900 uppercase tracking-tighter mb-2">
                    Shipping Method
                  </p>
            ...
            {!activeCustomer && (
              <div className="bg-amber-50 border border-amber-100 rounded-2xl p-6 flex flex-col md:flex-row items-start gap-4 shadow-sm">
                ...
                <div className="flex-1">
                  <h3 className="text-lg font-bold text-amber-900 mb-1 font-sans">
                  ...
                  <p className="text-amber-800 text-sm mb-3 leading-relaxed">
                    ...
                    is registered with this order. Create a password to easily
                    track this order and speed up future checkouts.
                  </p>
                  <a
            ...
            <div className="pt-10 flex flex-col sm:flex-row gap-4 border-t border-karima-brand/10">
              <a
                href="/"
                className="flex-1 bg-black hover:bg-karima-brand text-white text-center py-4 rounded-none font-black uppercase tracking-widest text-xs transition-all"
              >
                Back to Shopping
              </a>
              <a
                href="/account/history"
                className="flex-1 bg-white text-karima-brand border-2 border-karima-brand text-center py-4 rounded-none font-black uppercase tracking-widest text-xs hover:bg-karima-brand hover:text-white transition-all"
              >
                Order History
              </a>
            </div>
