...
import { data, redirect, useFetcher, useSearchParams, Link } from "react-router";
...
export async function action({ request }: Route.ActionArgs) {
    ...
    const { email, password, rememberMe, redirectTo = '/account' } = submission.value;
    ...
    try {
        ...
        if (result.__typename === 'CurrentUser') {
            ...
            return data(submission.reply({ formErrors: [result.message] }), { status: 401 });
        }
    } catch (error: any) {
                ...
                <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
                    <div className="bg-white/80 py-8 px-4 shadow-xl sm:rounded-lg sm:px-10 border border-karima-brand/10 backdrop-blur-sm">
                        ...
                        <loginFetcher.Form method="post" {...getFormProps(form)}>
                            <fieldset disabled={loginFetcher.state !== 'idle'} className="space-y-6">
                                <input
                                    type="hidden"
                                    name="redirectTo"
