...
export function WishlistButton({
  ...
  return (
    <button
      ...
      className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 shadow-sm ${className} ${
        localInWishlist
          ? "bg-white text-karima-brand"
          : "bg-white/80 text-gray-400 hover:bg-white hover:text-karima-brand"
      } ${isBusy ? "opacity-70 cursor-wait" : ""}`}
      title={localInWishlist ? "Remove from wishlist" : "Add to favorites"}
    >
      {localInWishlist ? (
        <HeartIconSolid className="h-5 w-5 flex-shrink-0" aria-hidden="true" />
      ) : (
        <HeartIcon className="h-5 w-5 flex-shrink-0" aria-hidden="true" />
      )}
      <span className="sr-only">
        {localInWishlist ? "Remove from wishlist" : "Add to favorites"}
      </span>
    </button>
  );
}
