...
export default function CartPage() {
  ...
  const isLoading =
    ...
    activeOrderFetcher.data === undefined;
  ...
  if (!activeOrder || lines.length === 0) {
    return (
      <div className="mt-32 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
        ...
        <Link
          to="/"
          className="inline-block bg-karima-brand text-white px-8 py-3 rounded-sm uppercase tracking-wider text-sm font-medium hover:bg-karima-brand/90 transition-colors"
        >
          Continue Shopping
        </Link>
      </div>
  ...
  return (
    <div className="mt-32 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <h1 className="text-4xl font-sans font-bold text-karima-brand mb-12">
        Your shopping cart
      </h1>
            ...
            {lines.map((line: any) => (
              <li key={line.id} className="flex py-6 sm:py-10">
                <div className="flex-shrink-0">
                  <img
                    src={(getVariantImagePreview(line.productVariant, line.featuredAsset) || "") + "?preset=thumb"}
                      ...
                      <div className="flex justify-between">
                        <h3 className="text-lg">
                          <Link
                            ...
                            className="font-medium text-gray-900 hover:text-karima-brand"
                          >
                        ...
                        {(line.discounts || []).some(
                          ...
                          <span className="flex flex-col items-start gap-1">
                            ...
                            <span className="text-green-600">
                              <Price
                                priceWithTax={
                                  line.proratedUnitPriceWithTax * line.quantity
                                }
                                currencyCode={currencyCode}
                              />
                            </span>
                          ...
                          <Price
                            priceWithTax={line.unitPriceWithTax * line.quantity}
                            currencyCode={currencyCode}
                          />
                        )}
                      </p>
          ...
          <div className="bg-gray-50/50 p-6 sm:p-8 rounded-none">
            <div className="flow-root">
              <dl className="-my-4 text-sm divide-y divide-gray-200">
                ...
                {(activeOrder?.discounts || [])
                  .filter((d: any) => d.amountWithTax < 0)
                  .map((discount: any, i: number) => (
                    <div
                      key={i}
                ...
                <div className="py-4 flex items-center justify-between">
                  ...
                  <dd className="font-medium text-gray-900">
                    <Price
                      priceWithTax={activeOrder?.taxTotal ?? 0}
                      currencyCode={currencyCode}
                    />
                  </dd>
                </div>
