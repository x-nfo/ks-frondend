...
export async function loader({ params, request, context }: Route.LoaderArgs) {
  ...
  try {
    ...
    const midtransPayment = order?.payments?.find((p) =>
    ...
    if (!order || !isSettled) {
      ...
      if (midtransPayment) {
        throw new Response("", {
          status: 302,
          headers: { Location: `/checkout/confirmation/${orderCode}` },
        });
      }
    }
  ...
  if (error || !order) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-16 text-center">
        <h1 className="text-2xl font-bold font-sans text-karima-brand mb-2">
