  ...
  try {
    switch (action) {
      case "searchDestinations": {
        ...
        const { rajaOngkirSearchDestinations } =
          await searchRajaOngkirDestinations(search, opts, limit, offset);
      ...
      case "calculateShipping": {
        const destinationId = parseInt(
          url.searchParams.get("destinationId") || "0",
          10,
        );
        ...
        const { rajaOngkirCalculateShipping } =
          await calculateRajaOngkirShipping(destinationId, weight, opts);

        return data({ shippingOptions: rajaOngkirCalculateShipping });
      }

      case "getCouriers": {
        const { rajaOngkirAvailableCouriers } =
          await getRajaOngkirCouriers(opts);
    ...
    return data(
      { error: "Failed to fetch data from RajaOngkir" },
      { status: 500 },
    );
  }
}
